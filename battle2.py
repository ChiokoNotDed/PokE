...
def Pencounter2(MyShiny):
    global Boss, Training, Trophy, DisablePB
    Trophy = False
    DisablePB = False
    Training = False
    Boss = False
...
def DamageCalculation():
    ...
    if MYNAME in [
      ...
      mdmg = 13
      mdef = 13
def Pencounter(Shiny):
    clear()
    global level, HP, x, dmg, sdef, mdmg, mdef, mpoison, mburn, mstun, mfreeze, mconfuse, epoison, eburn, estun, efreeze, econfuse, type1, type2, etype1, etype2, Battle, trainers, Trophy, move, emove, DisablePB
    ...
    if Boss == False:
        ...
        if db['AreaName'] == 'Eevee Land':
            ...
            x = random.choice([
                'Charizard', 'Flareon', 'Vibrava', 'Mightyena', 'Swellow',
                'Lairon', 'Manectric', 'Altaria', 'Ninetails', 'Luxio',
                'Gabite', 'Houndoom', 'Shroomish', 'Vulpix', 'Geodude',
                'Ponyta', 'Cyndaquil', 'Aipom', 'Snubbull', 'Slugma',
                'Skarmory', 'Torchic', 'Torkoal', 'Duskull', 'Begon', 'Buizel',
                'Bronzor', 'Riolu', 'Pidove', 'Milcery', "Poipole", 'Garchomp', 'Piethon',
            ])
            yt = random.randint(1, 2000)
            if yt == 1:
                x = 'Zekrom'
...
def Win(Level):
    global epoison, eburn, estun, efreeze, econfuse, Trophy
    epoison = False
    eburn = False
    estun = False
    efreeze = False
